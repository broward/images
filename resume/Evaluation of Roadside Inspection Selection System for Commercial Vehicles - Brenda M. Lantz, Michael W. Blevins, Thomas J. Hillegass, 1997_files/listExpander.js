...
$(this.itemSelector).css("display","block");$(this.itemSelector+":nth-child(n+"+(this.visible+this.offset)+")").addClass('not-show-important');$(this.itemSelector+":nth-child(-n+"+(this.visible+this.offset)+")").removeClass('not-show-important');$(this).trigger('blur');this.toggleButtons();if(itemCount===0){if($(this.moreSelector()).length){$(this.moreSelector())[0].focus();srSpeak($(this.moreSelector()).text());}}
...
makeExpandable(insertSelector){$(insertSelector).after('<div class="mediumButton grayButton" style="width:98%;margin:0;"><a id="'+this.hideId+'" tabindex="0" style="cursor:pointer;margin-top:-10px;"><span>Hide</span></a></div>');$(insertSelector).append('<div class="centeredRow"> \
		...
		</div>');let parent=this;$(this.moreSelector()).on('click',function(){parent.toggleItems(4);});$(this.hideSelector()).on('click',function(){parent.toggleItems(0);});$(this.allSelector()).on('click',function(){parent.toggleItems(5000);});$(this.moreSelector()+", "+this.hideSelector()+", "+this.allSelector()).on('keyup',function(e){if(e.keyCode===13){$(this).trigger('click');}});this.toggleItems(0);this.toggleButtons();if($(this.moreSelector()).length)
